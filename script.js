(() => {
  ...
  if (supportForm) {
    supportForm.addEventListener('submit', e => {
      ...
      toast('Спасибо! Поддержка Happ VPN ответит в течение 5 минут');
      supportForm.reset();
    });
  }
  ...
  if (meterFill && meterText) {
    setInterval(() => {
      const ping = 11 + Math.floor(Math.random() * 16);
      const speed = 820 + Math.floor(Math.random() * 160);
      const w = 78 + Math.floor(Math.random() * 18);
  ...
  /* ----- Year in footer (auto) ----- */
  const yearMatch = /© 2020[–-](\d{4})/;
  document.querySelectorAll('.footer__bottom span').forEach(s => {
    if (yearMatch.test(s.textContent)) {
      s.textContent = s.textContent.replace(yearMatch, `© 2020–${new Date().getFullYear()}`);
    }
  });
})();
